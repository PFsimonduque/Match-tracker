...
function Overlay({ children, wide }) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,
      ...
      <div style={{background:"#fff",borderRadius:20,padding:28,width:"100%",
...
// Asistencia: jugadores + opción sin asistencia
function AssistSelect({ players, onSelect, onClose }) {
  ...
  return (
    <Overlay>
      <h3 style={{margin:"0 0 14px",color:G.greenDark,fontFamily:"Georgia,serif",
        textAlign:"center",fontSize:20}}>🎯 ¿Quién asistió?</h3>
      <input placeholder="Buscar jugador..." value={q} onChange={e=>setQ(e.target.value)}
        style={{width:"100%",padding:"10px 14px",border:"2px solid #ddd",borderRadius:10,
          fontSize:16,fontFamily:"Georgia,serif",boxSizing:"border-box",marginBottom:12}}/>
      {/* Sin asistencia primero */}
      <button onClick={() => onSelect(null)}
        style={{display:"block",width:"100%",padding:"12px 14px",marginBottom:10,
          background:"#555",color:"white",border:"none",borderRadius:10,
          cursor:"pointer",fontFamily:"Georgia,serif",fontSize:15,fontWeight:"bold"}}>
        — Sin asistencia
      </button>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
        {filtered.map((p,i) => (
          <button key={i} onClick={() => onSelect(p)}
            style={{padding:"12px 14px",background:"#005588",color:"white",border:"none",
              borderRadius:10,cursor:"pointer",fontFamily:"Georgia,serif",fontSize:15,textAlign:"left"}}>
            {p.number ? `#${p.number} ` : ""}{p.name}
          </button>
        ))}
  ...
  return (
    <Overlay>
      <h3 style={{margin:"0 0 8px",color:G.greenDark,fontFamily:"Georgia,serif",
        textAlign:"center",fontSize:20}}>{label}</h3>
      <p style={{textAlign:"center",color:"#666",fontSize:14,margin:"0 0 14px"}}>Minuto del partido</p>
...
function SetupScreen({ onStart }) {
  ...
  const card = {background:"#fff",borderRadius:16,padding:24,marginBottom:16,
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#002500,#005500)",paddingBottom:50}}>
      <div style={{background:"#001a00",padding:"20px 24px",boxShadow:"0 4px 24px rgba(0,0,0,0.5)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:18,maxWidth:900,margin:"0 auto"}}>
          <img src={AN_SHIELD} style={{width:64,height:74,objectFit:"contain"}} alt="AN"
            onError={e=>{e.target.style.display="none"}}/>
          <div>
            <div style={{color:G.greenBright,fontFamily:"Georgia,serif",fontSize:26,fontWeight:"bold",letterSpacing:3}}>
              ATLÉTICO NACIONAL
            </div>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"22px 20px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
          <div style={card}>
            <h2 style={{margin:"0 0 16px",color:G.greenDark,fontFamily:"Georgia,serif",fontSize:19}}>📋 Información del Partido</h2>
            ...
            <h2 style={{margin:"18px 0 12px",color:G.greenDark,fontFamily:"Georgia,serif",fontSize:19}}>👔 Cuerpo Técnico</h2>
            {staff.map((s,i)=>(
          ...
          <div>
            <div style={card}>
              <h2 style={{margin:"0 0 12px",color:G.greenDark,fontFamily:"Georgia,serif",fontSize:19}}>🟢 Titulares</h2>
              {starters.map((p,i)=>(
            ...
            <div style={card}>
              <h2 style={{margin:"0 0 12px",color:G.greenDark,fontFamily:"Georgia,serif",fontSize:19}}>🔵 Suplentes</h2>
              {subs.map((p,i)=>(
                <div key={i} style={{display:"grid",gridTemplateColumns:"44px 1fr 70px",gap:6,marginBottom:6}}>
                  <input placeholder="#" style={{...inp,marginBottom:0,textAlign:"center",fontSize:14}}
...
function calcMinutesPlayed(player, events, matchEndMin) {
  // titulares entran en min 1, suplentes cuando los metan
  let entryMin = player.type === "titular" ? 1 : null;
  let exitMin = null;

  events.forEach(ev => {
    if (ev.type === "sub") {
      if (ev.in?.name === player.name) entryMin = ev.minute;
      if (ev.out?.name === player.name) exitMin = ev.minute;
    }
    if ((ev.type === "red") && ev.player?.name === player.name) {
      exitMin = ev.minute;
    }
    if ((ev.type === "injury") && ev.player?.name === player.name) {
      // puede haber salido por lesión + cambio, exitMin ya capturado por sub
    }
  });

  if (entryMin === null) return null; // nunca entró
  const end = exitMin !== null ? exitMin : matchEndMin;
  return Math.max(0, end - entryMin);
}
...
function LiveScreen({ matchData, onEnd }) {
  ...
  const ActionBtn = ({label,emoji,bg,action}) => (
    <button onClick={action}
      style={{padding:"18px 10px",background:bg,color:"white",border:"none",borderRadius:14,
        ...
        display:"flex",flexDirection:"column",alignItems:"center",gap:6,minHeight:90,
        boxShadow:"0 4px 14px rgba(0,0,0,0.25)",touchAction:"manipulation"}}>
  ...
  return (
    <div style={{height:"100vh",background:"#001500",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      {/* Top bar */}
      <div style={{background:"#000e00",padding:"12px 20px",boxShadow:"0 4px 20px rgba(0,0,0,0.6)",flexShrink:0}}>
        <div style={{maxWidth:980,margin:"0 auto",display:"flex",alignItems:"center",
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,flex:1}}>
            <img src={AN_SHIELD} style={{width:46,height:52,objectFit:"contain"}} alt="AN"
              onError={e=>{e.target.style.display="none"}}/>
            <div>
              <div style={{color:G.greenBright,fontFamily:"Georgia,serif",fontWeight:"bold",fontSize:15}}>Atlético Nacional</div>
          ...
          <div style={{textAlign:"center",flex:"0 0 auto"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:16,
              ...
              <span style={{color:"white",fontFamily:"Georgia,serif",fontSize:48,fontWeight:"bold",lineHeight:1}}>{score[0]}</span>
              <span style={{color:"#555",fontSize:28}}>–</span>
              <span style={{color:"white",fontFamily:"Georgia,serif",fontSize:48,fontWeight:"bold",lineHeight:1}}>{score[1]}</span>
            </div>
            <div style={{marginTop:4,display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
              <span style={{color:G.greenBright,fontFamily:"'Courier New',monospace",fontSize:24,fontWeight:"bold"}}>
                {timer.display}
              </span>
              <span style={{color:"rgba(255,255,255,0.4)",fontSize:12}}>{timer.half}° Tiempo</span>
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:8}}>
              {!timer.running
                ? <button onClick={timer.start}
                    style={{padding:"7px 18px",background:G.green,color:"white",border:"none",
                      borderRadius:20,cursor:"pointer",fontFamily:"Georgia,serif",fontWeight:"bold",fontSize:13}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,flex:1,justifyContent:"flex-end"}}>
            <div style={{textAlign:"right"}}>
              <div style={{color:"rgba(255,255,255,0.85)",fontFamily:"Georgia,serif",fontWeight:"bold",fontSize:15}}>
                {matchData.rival}
              </div>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",maxWidth:980,margin:"0 auto",
        width:"100%",padding:"16px 20px",gap:16,boxSizing:"border-box"}}>
        {/* Buttons */}
        <div style={{flex:"0 0 auto",display:"flex",flexDirection:"column",gap:10,width:280}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,flex:1}}>
            ...
            <button onClick={()=>onEnd(score,events,timer.matchMin)}
        ...
        <div style={{flex:1,background:"rgba(0,30,0,0.6)",borderRadius:16,padding:16,
          ...
          <h3 style={{margin:"0 0 12px",color:G.greenBright,fontFamily:"Georgia,serif",fontSize:17,flexShrink:0}}>
            📋 Eventos del partido
          </h3>
          <div style={{flex:1,overflowY:"auto"}}>
            {events.length===0
              ? <div style={{color:"rgba(255,255,255,0.3)",textAlign:"center",paddingTop:40,
                  fontFamily:"Georgia,serif",fontSize:15}}>Sin eventos aún</div>
              : [...events].reverse().map(ev=>(
                <div key={ev.id} style={{display:"flex",alignItems:"flex-start",gap:12,
                  ...
                  <span style={{color:G.greenBright,fontFamily:"'Courier New',monospace",
                    fontWeight:"bold",minWidth:42,fontSize:14,paddingTop:2}}>{ev.minute}&apos;</span>
                  <div style={{flex:1}}>
                    <div style={{color:"rgba(255,255,255,0.88)",fontFamily:"Georgia,serif",fontSize:14}}>
                    ...
                    {ev.type==="goal" && !ev.isOpponent && (
                      <div style={{color:"rgba(255,255,255,0.5)",fontFamily:"Georgia,serif",fontSize:12,marginTop:2}}>
                        {ev.assist ? `🎯 Asistencia: ${ev.assist.name}` : "Sin asistencia"}
                      </div>
                    )}
                  </div>
                  <span style={{color:"rgba(255,255,255,0.3)",fontSize:12,paddingTop:2}}>{ev.half}°T</span>
                </div>
              ))
            }
          </div>
...
// ── Report Screen ───────────────────────────────────────
function ReportScreen({ matchData, score, events, matchEndMin, onBack, onNewMatch }) {
  ...
  // Calcular minutos jugados para todos los jugadores
  const endMin = matchEndMin || 90;
  const playersWithMinutes = matchData.players
    .map(p => ({
      ...p,
      minutesPlayed: calcMinutesPlayed(p, events, endMin)
    }))
    .filter(p => p.minutesPlayed !== null)
  ...
  const badge = (min, bg=G.greenDark) =>
  ...
  // Bar for minutes played
  const MinBar = ({mins, max}) => (
    <div style={{flex:1,background:"#e8f5e9",borderRadius:4,height:10,margin:"0 8px",overflow:"hidden"}}>
      <div style={{background:G.green,height:"100%",width:`${Math.min(100,(mins/max)*100)}%`,
        borderRadius:4,transition:"width 0.3s"}}/>
    </div>
  );

  const maxMins = Math.max(...playersWithMinutes.map(p=>p.minutesPlayed||0), 1);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#eef2ee",paddingBottom:60}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",padding:"22px 20px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:28,marginBottom:16,flexWrap:"wrap"}}>
            <div style={{textAlign:"center"}}>
              <img src={AN_SHIELD} style={{width:76,height:86,objectFit:"contain"}} alt="AN"
                onError={e=>{e.target.style.display="none"}}/>
              <div style={{fontSize:12,color:G.greenBright,fontWeight:"bold",marginTop:6}}>Atlético Nacional</div>
            </div>
            <div>
            ...
            <div style={{textAlign:"center"}}>
              {matchData.rivalLogo
                ...
                : <div style={{width:76,height:86,background:"rgba(255,255,255,0.06)",borderRadius:10,
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,opacity:0.4}}>Rival</div>
              }
              <div style={{fontSize:12,color:G.greenBright,fontWeight:"bold",marginTop:6}}>{matchData.rival}</div>
            </div>
          </div>
              ...
              : goals.map((g,i)=>(
                <div key={i} style={{padding:"10px 0",borderBottom:"1px solid #f0f0f0"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    {badge(g.minute, g.isOpponent?"#880000":G.greenDark)}
                  ...
                  {!g.isOpponent && (
                    <div style={{marginTop:4,marginLeft:52,color:"#666",fontFamily:"Georgia,serif",fontSize:13}}>
          ...
          <div>
            {(yellows.length>0||reds.length>0) && (
              <div style={card}>
                ...
                {yellows.map((y,i)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"7px 0",borderBottom:"1px solid #f0f0f0"}}>
                    {badge(y.minute,"#AA7700")}
                    <span style={{fontFamily:"Georgia,serif",fontSize:14}}>🟨 {y.player?.name}</span>
                  </div>
                ))}
                {reds.map((r,i)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"7px 0",borderBottom:"1px solid #f0f0f0"}}>
                    {badge(r.minute,"#880000")}
            ...
            {subs.length>0 && (
              <div style={card}>
                ...
                {subs.map((s,i)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"7px 0",borderBottom:"1px solid #f0f0f0"}}>
                    {badge(s.minute,"#003366")}
                    <span style={{fontFamily:"Georgia,serif",fontSize:14}}>⬇️ {s.out?.name} → ⬆️ {s.in?.name}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {/* Minutos jugados — full width */}
        {playersWithMinutes.length > 0 && (
          <div style={card}>
            <div style={st}>⏱ Minutos Jugados</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 16px"}}>
              {playersWithMinutes.map((p,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",
                  borderBottom:"1px solid #f5f5f5"}}>
                  {p.number&&<span style={{background:p.type==="titular"?G.greenDark:"#003366",
                    color:"white",borderRadius:4,padding:"1px 5px",fontWeight:"bold",
                    fontSize:11,minWidth:22,textAlign:"center"}}>{p.number}</span>}
                  <span style={{fontFamily:"Georgia,serif",fontSize:13,minWidth:110,flex:"0 0 auto"}}>
                    {p.name}
                  </span>
                  <MinBar mins={p.minutesPlayed} max={maxMins}/>
                  <span style={{fontFamily:"'Courier New',monospace",fontWeight:"bold",
                    fontSize:13,color:G.greenDark,minWidth:36,textAlign:"right"}}>
                    {p.minutesPlayed}&apos;
                  </span>
                </div>
              ))}
          ...
          {matchData.starters?.length>0 && (
            <div style={card}>
              ...
              {matchData.starters.map((p,i)=>(
                <div key={i} style={{display:"flex",gap:8,alignItems:"center",padding:"6px 8px",
                  background:"#f6fff6",borderRadius:7,border:"1px solid #d4ecd4",marginBottom:5}}>
                  {p.number&&<span style={{background:G.greenDark,color:"white",borderRadius:4,
                    padding:"1px 6px",fontWeight:"bold",fontSize:12,minWidth:24,textAlign:"center"}}>{p.number}</span>}
            ...
            {matchData.subs?.length>0 && (
              <div style={card}>
                ...
                {matchData.subs.map((p,i)=>(
                  <div key={i} style={{display:"flex",gap:8,alignItems:"center",padding:"6px 8px",
                    background:"#f5f8ff",borderRadius:7,border:"1px solid #d0dcf0",marginBottom:5}}>
        ...
        <div style={{textAlign:"center",padding:"18px 0",color:"#aaa",fontFamily:"Georgia,serif",fontSize:13}}>
...
// ── Persistence ─────────────────────────────────────────
const SK = "an_match_v4";
const loadH = () => { try { return JSON.parse(localStorage.getItem(SK)||"[]"); } catch { return []; } };
const saveH = r => {
  try { const h=loadH(); h.unshift({...r,id:Date.now()}); localStorage.setItem(SK,JSON.stringify(h.slice(0,20))); }
  catch {}
};
...
export default function App() {
  ...
  const [score, setScore] = useState([0,0]);
  const [events, setEvents] = useState([]);
  const [matchEndMin, setMatchEndMin] = useState(90);
  ...
  const handleEnd = (s,e,endMin) => {
    setScore(s); setEvents(e); setMatchEndMin(endMin||90);
    if (matchData) { saveH({matchData,score:s,events:e,matchEndMin:endMin||90}); setHistory(loadH()); }
    setScreen("report");
  };

  if (histDetail) return (
    <ReportScreen matchData={histDetail.matchData} score={histDetail.score}
      events={histDetail.events} matchEndMin={histDetail.matchEndMin||90}
  ...
  if (screen==="report") return (
    <ReportScreen matchData={matchData} score={score} events={events} matchEndMin={matchEndMin}
            ...
            {history.length===0
              ...
              : history.map(h=>(
                <div key={h.id} onClick={()=>{setHistDetail(h);setShowH(false);}}
                  ...
                  <div style={{fontFamily:"Georgia,serif",fontWeight:"bold",fontSize:16,color:G.greenDark}}>
